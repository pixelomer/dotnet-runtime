#!/usr/bin/env python3
"""Build production PAL executable memory and cache publication with Horizon tests.
...
for line in (build / "minipal/Unix/CMakeFiles/coreclrminipal_objects.dir/flags.make").read_text().splitlines():
    if " = " in line:
        name, value = line.split(" = ", 1)
        mini_flags[name] = shlex.split(value)
compile_flags = flags['CXX_DEFINES'] + flags['CXX_INCLUDES'] + flags['CXX_FLAGS']
objects = []
for unit in [source / 'main.cpp', repo / 'src/coreclr/minipal/libnx/doublemapping.cpp',
             repo / 'src/coreclr/pal/src/thread/context.cpp']:
    ...
    if unit.name == "doublemapping.cpp":
        unit_flags = mini_flags["CXX_DEFINES"] + mini_flags["CXX_INCLUDES"] + mini_flags["CXX_FLAGS"]
