#!/usr/bin/env python3
"""Build production PAL exception dispatch and context restoration with Horizon tests.
...
output = repo / 'artifacts/libnx-coreclr-exceptions'
output.mkdir(parents=True, exist_ok=True)
compile_flags = flags['CXX_DEFINES'] + flags['CXX_INCLUDES'] + flags['CXX_FLAGS']
objects = []
for unit in [source / 'main.cpp', source / 'fault.S']:
...
# Select only objects belonging to the current CMake target (exclude stale
# objects left by an earlier platform source list). The not-yet-ported native
# module loader may be absent; every linked dependency must still resolve.
...
units = [unit for unit in units if unit.is_file()]
...
target = output / 'coreclr-exception-probe'
...
subprocess.run([str(devkitpro / 'tools/bin/nacptool'), '--create', 'CoreCLR exception bridge',
