#!/usr/bin/env python3
"""Cross-build the original native shared allocator regression probe; no device action."""
import argparse
import hashlib
import json
...
target = out / 'protected-pool'
common = Path(__file__).resolve().parents[6] / 'src/native/libs/Common'
cmd = ['aarch64-none-elf-gcc', '-O2', '-g', '-march=armv8-a+crc+crypto',
       ...
       '-Wl,-Map,' + str(target.with_suffix('.map')), str(source), str(common / 'nxvm.c'), '-I' + str(common), '-Wl,--wrap=svcSetMemoryPermission',
       '-L' + str(sdk / 'lib'), '-lnx', '-o', str(target.with_suffix('.elf'))]
subprocess.run(cmd, check=True)
subprocess.run(['nacptool', '--create', 'Horizon protected pool probe', 'Homebrew runtime research',
...
(out / 'manifest.json').write_text(json.dumps(dict(command=cmd, source_sha256=sha(source), nxvm_sha256=sha(common / 'nxvm.c'),
