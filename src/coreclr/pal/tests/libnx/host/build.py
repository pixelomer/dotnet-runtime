...
subprocess.run([str(compiler), '-march=armv8-a+crc+crypto', '-mtune=cortex-a57', '-mtp=soft', '-fPIE',
                ...
                '-Wl,-Map,' + str(target.with_suffix('.map')), '-Wl,--eh-frame-hdr', '-Wl,--wrap=VirtualProtect', '-Wl,--wrap=virtmemFindStack', '-Wl,--wrap=CreateFileW', '-Wl,--wrap=WideCharToMultiByte', '-Wl,--export-dynamic-symbol=coreclr_initialize',
