...
{
    FILE* file=fopen("sdmc:/switch/nativeaot10-managed-stress.txt","a");
    if (!file) abort();
...
int main()
{
    FILE* file=fopen("sdmc:/switch/nativeaot10-managed-stress.txt","w");
    if (!file) return 1;
    fprintf(file,"BEGIN native launcher; source-built Horizon NativeAOT 10.0.12\n");
    fclose(file);
    int result=0;
    for(int round=0;round<16;round++) {
        result=ManagedStressMain();
        FILE* progress=fopen("sdmc:/switch/nativeaot10-managed-stress.txt","a");
        if (!progress) abort();
    ...
    file=fopen("sdmc:/switch/nativeaot10-managed-stress.txt","a");
    if (!file) return 1;
