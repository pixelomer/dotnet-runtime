#!/usr/bin/env python3
"""Build the Horizon managed filesystem object, then link only Horizon native libraries."""
from pathlib import Path
import os,subprocess,re,shutil,json,hashlib
here=Path(__file__).resolve().parent
repo=here.parents[6]
out=repo/'artifacts/libnx-filesystem-test'
...
(out/'FileSystem.csproj').write_text('''<Project Sdk="Microsoft.NET.Sdk">
<PropertyGroup>
...
shutil.copy2(here/'FileSystem.cs',out/'FileSystem.cs')
with (out/'publish.log').open('w') as log:
    subprocess.run(['dotnet','publish',str(out/'FileSystem.csproj'),'-c','Release','-r','linux-arm64','--self-contained','-p:IlcSdkPath='+str(sdk)+'/'],cwd=out,stdout=log,stderr=subprocess.STDOUT,check=True)
if re.search(r'warning IL\d+', (out/'publish.log').read_text()):raise SystemExit('AOT warnings remain')
obj=out/'obj/Release/net9.0/linux-arm64/native/FileSystem.o'
...
cmd=[str(dkp/'devkitA64/bin/aarch64-none-elf-g++'),*flags,str(here/'main.cpp'),str(obj),str(sdk/'libbootstrapperdll.o'),'-specs='+str(out/'switch.specs'),'-Wl,--eh-frame-hdr,-Map,'+str(out/'filesystem.map'),'-Wl,--start-group',str(sdk/'libRuntime.WorkstationGC.a'),str(sdk/'libeventpipe-disabled.a'),str(sdk/'libstandalonegc-disabled.a'),str(libs/'libSystem.Native.a'),str(libs/'libSystem.Globalization.Native.a'),str(libs/'libSystem.IO.Compression.Native.a'),str(icu/'lib/libicui18n.a'),str(icu/'lib/libicuuc.a'),str(icu/'lib/libicudata.a'),'-L'+str(dkp/'portlibs/switch/lib'),'-L'+str(dkp/'libnx/lib'),'-lz','-lnx','-Wl,--end-group','-o',str(out/'nativeaot-filesystem-test.elf')]
...
subprocess.run([str(dkp/'tools/bin/elf2nro'),str(out/'nativeaot-filesystem-test.elf'),str(out/'nativeaot-filesystem-test.nro')],check=True)
inputs=[*sorted(sdk.glob('*.dll')),obj,sdk/'libRuntime.WorkstationGC.a',sdk/'libbootstrapperdll.o',
        ...
        *sorted(libs.glob('*.a')),out/'nativeaot-filesystem-test.nro',out/'switch.ld',out/'switch.specs']
manifest={
    ...
    'source_sha256':{str(p.relative_to(repo)):hashlib.sha256(p.read_bytes()).hexdigest()
        for p in [here/'build.py',here/'main.cpp',here/'FileSystem.cs',here.parent.parent/'create-linker-script.py']},
    'managed_linux_tls_instructions_or_relocations':0,
    'managed_il_warnings':0,
}
(out/'manifest.json').write_text(json.dumps(manifest,indent=2)+'\n')
print(out/'nativeaot-filesystem-test.nro')
