...
For .NET 10, retain the matching native compression dependencies from the
source build, including zlib and the Brotli encoder, decoder and common archives;
they are not part of this SDK inventory. The .NET 10 package does not include
System.Net.Sockets.dll. Socket applications must build the matching platform
framework from the source checkout with:

```sh
./build.sh libs.sfx -os libnx -arch arm64 -c Release /p:RuntimeFlavor=CoreCLR /p:PublicSign=true
```

Select its System.Net.Sockets.dll from
artifacts/bin/runtime/net10.0-libnx-Release-arm64 explicitly during compilation,
following the source checkout's networking-async probe recipe. Use the same
source-built SDK, native archives and framework version throughout.
